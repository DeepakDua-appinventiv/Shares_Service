...
import { GetCompanyRequest, GetCompanyResponse, GetShareRequest, GetShareResponse } from './shares.pb';
...
export class SharesService implements OnModuleInit {
  ...
  public async searchCompany(payload: GetCompanyRequest): Promise<GetCompanyResponse> {
    try {
  ...
  public async getShare(payload: any) {
  try {
    console.log(payload);
    const companyId = payload;
    console.log(companyId);
    const shares:GetShareResponse = await firstValueFrom(this.svc.getShare({companyId: companyId }));
    return { status: 200, shares, error: [] };
  } catch (error) {
